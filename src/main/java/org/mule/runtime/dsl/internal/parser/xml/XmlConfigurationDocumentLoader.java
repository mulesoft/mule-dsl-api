...
public final class XmlConfigurationDocumentLoader {
  ...
  private XmlConfigurationDocumentLoader(XmlGathererErrorHandlerFactory errorHandlerFactory) {
    this.enableValidations = errorHandlerFactory != null;
   ...
   * @throws MuleRuntimeException if the current {@code filename} contains 1 or more errors.
  ...
  public Document loadDocument(Set<ExtensionModel> extensions, String filename, InputStream inputStream) {
    ...
    Document document;
    try {
      //TODO check whay the XSD validation changed the entity resolver to be a DelegatingEntityResolver
      document = new MuleDocumentReader()
          .loadDocument(new InputSource(inputStream), new ModuleDelegatingEntityResolver(extensions),
  ...
  private XmlGathererErrorHandler createXmlGathererErrorHandler() {
    return enableValidations ? xmlGathererErrorHandlerFactory.create() : null;
  }
}
