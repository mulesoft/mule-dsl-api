...
import static org.mule.runtime.dsl.api.xml.CoreDslConstants.IMPORT_ELEMENT;
...
import org.mule.runtime.dsl.api.xml.StaticXmlNamespaceInfo;
import org.mule.runtime.dsl.api.xml.StaticXmlNamespaceInfoProvider;
...
public class XmlArtifactParser {
  ...
  public ArtifactDefinition parse() {
    try {
    ...
    List<Pair<String, InputStream>> newConfigFilesToResolved = importedFiles.stream()
        .map(importedFileName -> {
          ...
          if (resourceAsStream == null) {
            throw new MuleRuntimeException(createStaticMessage(format("Could not find imported resource '%s'",
                                                                      importedFileName)));
          }
  ...
  private static List<XmlNamespaceInfoProvider> discoverRuntimeXmlNamespaceInfoProvider() {
    ...
    namespaceInfoProvidersBuilder
        .addAll(lookupProviders(XmlNamespaceInfoProvider.class,
